...
public class ShopNPC extends CustomNPC {
    ...
    public void interact(PlayerData playerData) {
        ...
        playerData.openGUI(new VendorGUI(playerData, "Shop NPC's Shop",
                new PurchasableItem(CustomRecipeScrolls.RECIPE_SCROLL_SLIME_TO_TRAINING_WAND.get(), ItemUtil.Value(0,0,25)),
                new PurchasableItem(CustomEquipment.MISTWEAVE_ROBE.get(), ItemUtil.Value(2,0,0)),
                new PurchasableItem(CustomArrows.SIMPLE_ARROW.get(), ItemUtil.Value(0,0,5), 5)
                ));
    }
}
